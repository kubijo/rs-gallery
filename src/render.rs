...
/// A capture recipe for one scene, with every stateful knob written out at its current value.
///
...
fn capture_template(scene: &SceneEntry, knobs: &[Knob], size: egui::Vec2, scale: f32) -> String {
    ...
    let mut out = formatdoc! {r#"
        # Generated by `--init-capture`: every stateful knob at the value its scene declared.
    ...
    for knob in knobs {
        match knob {
            ...
            // A click is an interactive event, not state a deterministic capture can restore.
            Knob::Button { .. } => {}
            _ => {
...
fn toml_value(knob: &Knob) -> String {
    match knob {
        Knob::Button { .. } => unreachable!("a button carries no value to write as TOML"),
