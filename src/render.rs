...
pub(crate) fn render_with_globals(
    ...
    report(
        &written,
        skipped.as_ref(),
        capture.shots.iter().any(|shot| shot.template),
    );
    outcome
}
...
fn report(written: &[OutputFile], skipped: Option<&Diagnostic>, recipe_on_stdout: bool) {
    if !written.is_empty() {
        ...
        let summary = frame(&headline, &rows(written), Some(&total));
        if recipe_on_stdout {
            anstream::eprintln!("\n{summary}");
        } else {
            anstream::println!("\n{summary}");
...
fn set(knob: &mut Knob, raw: &str) -> Result<(), String> {
    ...
    match knob {
        Knob::Button { .. } => {
            return Err(format!("`{label}` is a button action and carries no value"));
        }
...
fn describe(knob: &Knob) -> String {
    match knob {
        Knob::Button { label, .. } => format!("button  {label:?}"),
...
fn capture_template_with_globals(
    ...
    let mut out = formatdoc! {r#"
        # Generated by `--init-capture`: every stateful control at its current value.
        out = "renders"
        size = "{width}x{height}"
    ...
    out.push_str("\n[[shot]]\n");
    out.push_str(
        &toml::to_string(&BTreeMap::from([("name", name), ("scene", key)]))
            .expect("scene identifiers are TOML strings"),
    );
...
fn write_controls(out: &mut String, table: &str, knobs: &[Knob]) {
    ...
    for knob in knobs {
        if let Knob::Group { label } = knob {
            out.push('\n');
            for line in label.lines() {
                let _ = writeln!(out, "# {}", line.escape_debug());
            }
        } else if let Some(value) = control_value(knob) {
            out.push_str(
                &toml::to_string(&BTreeMap::from([(label(knob), value)]))
                    .expect("control values are TOML scalars"),
            );
        }
    }
}
...
fn control_value(knob: &Knob) -> Option<ControlValue> {
    Some(match knob {
        ...
        Knob::Toggle { value, .. } => ControlValue::Toggle(*value),
        Knob::Slider { value, .. } => ControlValue::Slider(*value),
        Knob::Text { value, .. } => ControlValue::Text(value.clone()),
        Knob::Color { value, .. } => ControlValue::Text(value.to_hex()),
        Knob::Select { value, options, .. } | Knob::IconButtons { value, options, .. } => {
            ControlValue::Text(options.get(*value).cloned().unwrap_or_default())
        }
        Knob::Pad2D { x, y, .. } => ControlValue::Text(format!("{x},{y}")),
    })
}
...
mod tests {
    ...
    #[test]
    fn generated_recipes_round_trip_escaped_labels_and_values() {
        let scene = crate::test_support::scene("escaped", "capture", true);
        for value in [
            "a=b",
            "'\"\\",
            "line\nreturn\rtab\t",
            ...
            let recipe: Recipe =
                toml::from_str(&generated).expect("generated recipe is valid TOML");
            assert_eq!(recipe.globals[&label].as_str(), Some(value));
            assert_eq!(recipe.shots[0].knobs[&label].as_str(), Some(value));
            assert_eq!(recipe.shots[0].knobs["choice"].as_str(), Some(value));
        }
    }
    ...
    #[test]
    fn a_generated_recipe_states_every_value_and_omits_actions() {
    ...
    fn a_generated_recipe_reads_back_as_the_state_it_was_generated_from() {
        ...
        for knob in &mut store {
            match knob {
                Knob::Button { clicked, .. } => *clicked = false,
    ...
    #[cfg(not(target_vendor = "apple"))]
    #[test]
    fn a_tall_capture_keeps_its_full_png_and_uploads_a_smaller_sheet_panel() {
        fn tall(ctx: &mut crate::SceneCtx<'_>, ui: &mut egui::Ui) {
            ctx.stage(ui, (120.0, 2600.0), |ui| {
                ui.painter()
                    .rect_filled(ui.max_rect(), 0.0, egui::Color32::RED);
                let mut bottom = ui.max_rect();
                bottom.min.y = bottom.max.y - 100.0;
                ui.painter().rect_filled(bottom, 0.0, egui::Color32::BLUE);
            });
        }
        let manifest = Manifest {
            scenes: vec![SceneEntry {
                render: tall,
                ..crate::test_support::scene("tall", "capture", true)
            }],
            groups: Vec::new(),
        };
        let temp = tempfile::tempdir().expect("capture directory");
        let dir = Utf8Path::from_path(temp.path()).expect("UTF-8 path");
        ...
        .unwrap();
        let capture = read_recipe(&recipe, None).expect("a tall capture recipe");
        render(
            &manifest,
            Renderer::Glow,
            &|ctx| {
                ctx.input_mut(|input| input.max_texture_side = 2048);
            },
            &capture,
        )
        .expect("tall shots and their sheet render");
        let out = dir.join("renders");
        let image = image::open(out.join("one.png"))
            .expect("full shot PNG")
            .to_rgba8();
        assert!(
            image.height() >= 2600,
            "the whole stage is captured: {}",
            image.height()
        );
        assert!(
            image
                ...
                .skip(2500)
                .flatten()
                .any(|pixel| pixel.0 == [0, 0, 255, 255]),
            "the bottom of the stage is painted too"
        );
        let sheet = image::open(out.join("sheet.png"))
            .expect("sheet PNG")
            .to_rgba8();
        assert!(
            sheet.height() >= image.height(),
            "the packed display size survives texture downsampling"
        );
        assert!(
            sheet
                .rows()
                ...
                .flatten()
                .any(|pixel| pixel.0 == [0, 0, 255, 255]),
            "the sheet includes the bottom of the resized panel"
        );
        let report: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(out.join("capture.json")).unwrap())
                .unwrap();
        assert_eq!(report["complete"], true);
        assert!(report["sheet"].as_str().unwrap().ends_with("sheet.png"));
        for shot in report["shots"].as_array().unwrap() {
            assert_eq!(shot["height"], image.height());
        }
    }
    ...
    fn a_listing_quotes_the_label_and_names_the_accessor_that_declared_it() {
        ...
        assert_eq!(
            describe(&Knob::Button {
                label: "Reset state".to_owned(),
                clicked: false,
            }),
            r#"button  "Reset state""#
        );
    }
